...
assert.match(mpeg, /Waiting for origin/);
...
assert.match(src("mpegTsPlayback.ts"), /mpegTsHoldReconnectsWhileJobRunning/);
assert.match(src("mpegTsPlayback.ts"), /HTTP 404/);
...
assert.match(py("tests/test_publisher_protocol.py"), /34-138-137-211\.sslip\.io:14433/);
assert.match(py("tests/test_publisher_protocol.py"), /--insecure-skip-verify/);
...
assert.match(src("mpegTsPlayback.ts"), /mpegTsEmptyReplyReason/);
assert.match(src("mpegTsPlayback.ts"), /empty-reply/);
assert.match(src("players/MpegTsPlayer.tsx"), /emptyReply: idle.emptyReply/);
assert.match(py("web/api/main.py"), /playback_fetch_empty_reply_response/);
assert.match(py("web/api/main.py"), /X-Playback-First-Byte": "empty-reply"/);
assert.match(py("web/api/main.py"), /read=120\.0/);
assert.match(py("src/upload_service.py"), /looks_like_flv_vtag_mismatch/);
assert.match(py("src/upload_service.py"), /flv_vtag_remux_fail_message/);
assert.match(py("src/upload_service.py"), /unknown option 'vtag'/);
assert.match(src("moqCmafPlayback.ts"), /FLV vtag 27/);
assert.match(src("moqCmafPlayback.ts"), /isFlvVtagRemuxMiss/);
assert.match(src("moqCmafPlayback.ts"), /unknown option \['"\]\?vtag/);
assert.match(src("comparisonReplay.ts"), /playerErrorForFailedJob\(\{\s*jobStatus: hud\.jobStatus,\s*jobError: hud\.jobError,\s*protocol,/);
assert.match(src("comparisonReplay.test.ts"), /2026-09-01 Cloud\/Edge Zixi\+MTX Lavf63/);
assert.match(src("comparisonReplay.test.ts"), /Nothing was written into output file/);
assert.match(py("src/encoder_capture.py"), /-tag:v", "7"/);
assert.match(py("src/encoder_capture.py"), /-f", "flv"/);
assert.match(py("src/encoder_capture.py"), /-c:a", "copy"/);
assert.doesNotMatch(py("src/encoder_capture.py"), /f=flv:vtag=7/);
assert.doesNotMatch(py("src/encoder_capture.py"), /vtag=/);
assert.doesNotMatch(py("src/encoder_capture.py"), /f=flv:/);
...
assert.match(src("recipeSupport.ts"), /if \(webcamSixWayFanout\(endpoints, ctx\)\)/);
...
assert.match(src("moqCmafPlayback.ts"), /write-blocked|dropping fragment/);
assert.match(src("comparisonReplay.ts"), /visibleMoqError/);
assert.match(py("src/moq_publish.py"), /def publisher_fragment_drop_count/);
assert.match(py("src/moq_publish.py"), /def publisher_write_block_error/);
assert.match(py("src/moq_publish.py"), /def should_reencode_brokered_moq/);
assert.match(py("src/upload_service.py"), /publisher_write_block_error/);
assert.match(py("src/upload_service.py"), /dest_count=/);
assert.match(src("comparisonReplay.ts"), /protocol === "srt" \|\| protocol === "rtmp"/);
assert.match(src("comparisonReplay.test.ts"), /East SRT Test.ts empty-reply/);
assert.match(src("comparisonReplay.test.ts"), /DASH MPD unavailable/);
...
assert.match(hls, /playerErrorForFailedJob/);
assert.match(hls, /jobError/);
assert.doesNotMatch(hls, /SRT encode job failed \(0 kbps\)/);
...
assert.match(streamPlayer, /target\.engine === "hls"[\s\S]*jobError=\{jobError\}/);
assert.match(app, /leg\?\.job\.endpoint_url \|\| resolveEndpointUrl/);
assert.match(src("comparisonReplay.test.ts"), /173\.230\.155\.121:8888\/benchmark\/index\.m3u8/);
assert.match(src("comparisonReplay.test.ts"), /cloud-compare RTMP BBB MTX HLS 404/);
assert.match(src("comparisonReplay.ts"), /playerErrorForFailedJob/);
assert.match(src("recipeMatrix.test.ts"), /cloud-compare RTMP BBB fans five dests/);
assert.match(py("src/moq_publish.py"), /def apply_mediamtx_rtmp_job_path/);
assert.match(py("web/api/job_manager.py"), /apply_mediamtx_rtmp_job_path/);
assert.match(py("src/upload_service.py"), /MediaMtxStatsPoller\._path_from_url\(job\.destination\.url\)/);
