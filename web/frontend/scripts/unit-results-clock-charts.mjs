...
assert.match(sourceSrc, /This computer’s camera/);
assert.match(sourceSrc, /A file on the server/);
assert.match(sourceSrc, /source-mode-advanced/);
...
assert.match(chartsSrc, /showLegend=\{false\}/);
assert.doesNotMatch(cssSrc, /\.benchmark-live\.has-charts/);
assert.match(cssSrc, /\.benchmark-page/);
assert.match(appSrc, /className="benchmark-page"/);
assert.doesNotMatch(appSrc, /has-charts/);
const metricChartSrc = fs.readFileSync(path.join(root, "src/MetricChart.tsx"), "utf8");
assert.match(metricChartSrc, /const numericSeries/);
assert.match(metricChartSrc, /showLegend/);
assert.doesNotMatch(metricChartSrc, /nonzeroSeries/);
