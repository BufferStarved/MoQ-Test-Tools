/**
 * Tests for parseMsfCatalog() — independent catalog parsing and validation.
 *
...
describe('parseMsfCatalog', () => {
    ...
    it('parses §5.3.1 — audio/video single quality', () => {
        const catalog = parseMsfCatalog(EXAMPLE_AV_SINGLE);
        expect(catalog.version).toBe(1);
    ...
    it('parses §5.3.2 — simulcast with altGroup', () => {
        const catalog = parseMsfCatalog(EXAMPLE_SIMULCAST);
    ...
    it('parses §5.3.3 — SVC with depends', () => {
        const catalog = parseMsfCatalog(EXAMPLE_SVC);
    ...
    it('parses §5.3.7 — VOD tracks (isLive=false, trackDuration)', () => {
        const catalog = parseMsfCatalog(EXAMPLE_VOD);
        expect(catalog.tracks).toHaveLength(2);
    ...
    it('parses §5.3.9 — broadcast termination (isComplete, empty tracks)', () => {
        const catalog = parseMsfCatalog(EXAMPLE_TERMINATE);
    ...
    it('parses §5.3.6 — ignores custom fields (§5.1)', () => {
        const catalog = parseMsfCatalog(EXAMPLE_CUSTOM_FIELDS);
        expect(catalog.tracks).toHaveLength(2);
    ...
    it('parses §5.3.8 — timeline tracks (mediatimeline + eventtimeline)', () => {
        const catalog = parseMsfCatalog(EXAMPLE_TIMELINES);
        expect(catalog.tracks).toHaveLength(4);
    ...
    it('accepts Uint8Array input', () => {
        ...
        const catalog = parseMsfCatalog(bytes);
        expect(catalog.version).toBe(1);
    });
    ...
    it('inherits catalog namespace when track namespace is absent (§5.1.10)', () => {
        const catalog = parseMsfCatalog(EXAMPLE_SIMULCAST, 'live.example.com/broadcast1');
    ...
    it('track-level namespace overrides inherited namespace (§5.1.10)', () => {
        ...
        const catalog = parseMsfCatalog(json, 'default/ns');
    ...
    it('rejects missing tracks array (§5.1.8)', () => {
        expect(() => parseMsfCatalog('{"version": 1}')).toThrow(/tracks/i);
    });
    ...
    it('rejects track without name (§5.1.11)', () => {
        ...
        expect(() => parseMsfCatalog(json)).toThrow(/name/i);
    });
    ...
    it('accepts cmaf packaging (draft-ietf-moq-cmsf-00 §3.5.1)', () => {
        ...
        const catalog = parseMsfCatalog(json);
    ...
    it('rejects track without packaging (§5.1.12)', () => {
        ...
        expect(() => parseMsfCatalog(json)).toThrow(/packaging/i);
    });
    ...
    it('rejects track without isLive (§5.1.15)', () => {
        ...
        expect(() => parseMsfCatalog(json)).toThrow(/isLive/i);
    });
    ...
    it('rejects duplicate track names within same namespace (§5.1.11)', () => {
        ...
        expect(() => parseMsfCatalog(json)).toThrow(/unique|duplicate/i);
    });
    ...
    it('rejects targetLatency when isLive=false (§5.1.16)', () => {
        ...
        expect(() => parseMsfCatalog(json)).toThrow(/targetLatency/i);
    });
    ...
    it('rejects trackDuration when isLive=true (§5.1.37)', () => {
        ...
        expect(() => parseMsfCatalog(json)).toThrow(/trackDuration/i);
    });
    ...
    it('rejects eventType with non-eventtimeline packaging (§5.1.13)', () => {
        ...
        expect(() => parseMsfCatalog(json)).toThrow(/eventType/i);
    });
    ...
    it('rejects invalid JSON', () => {
        expect(() => parseMsfCatalog('not json')).toThrow();
    });
    ...
    it('rejects isComplete=false — MUST NOT be included if false (§5.1.7)', () => {
        ...
        expect(() => parseMsfCatalog(json)).toThrow(/isComplete/i);
    });
    ...
    it('rejects parentName in independent catalog track (§5.1.36)', () => {
        ...
        expect(() => parseMsfCatalog(json)).toThrow(/parentName/i);
    });
    ...
    it('rejects mediatimeline track without mimeType="application/json" (§7.2)', () => {
        ...
        expect(() => parseMsfCatalog(json)).toThrow(/mimeType/i);
    });
    ...
    it('accepts valid BCP 47 lang tags (§5.1.35)', () => {
        ...
        for (const lang of validTags) {
            ...
            expect(() => parseMsfCatalog(json)).not.toThrow();
        }
    });
    ...
    it('rejects invalid lang tag: empty string (§5.1.35)', () => {
        ...
        expect(() => parseMsfCatalog(json)).toThrow(/lang/i);
    });
    ...
    it('rejects invalid Base64 initData: special characters (§5.1.20)', () => {
        ...
        expect(() => parseMsfCatalog(json)).toThrow(/initData/i);
    });
    ...
    it('preserves initTrack field on output (CMSF §3.5.1)', () => {
        const catalog = parseMsfCatalog(new TextEncoder().encode(JSON.stringify({
            version: 1,
            tracks: [{
                name: 'video', packaging: 'cmaf', role: 'video',
                codec: 'avc1.64001f', width: 1920, height: 1080,
                isLive: true,
                initTrack: 'init-segment',
            }],
        })));
        expect(catalog.tracks[0]!.initTrack).toBe('init-segment');
    });
    ...
    it('parses CMSF catalog with SAP eventtimeline tracks (real-world scenario)', () => {
        // Regression test for: "CMSF SAP tracks appear to fail on the simple player"
        // The catalog from the issue has 4 tracks: 2 CMAF media + 2 SAP eventtimeline.
        // The player must parse all tracks, identify the SAP tracks by packaging and
        // eventType, and subscribe to them alongside the media tracks.
        // @see draft-ietf-moq-cmsf-00 §3.6 (SAP Type Timeline)
        // @see draft-ietf-moq-msf-00 §8.2 (Event Timeline Catalog requirements)
        const catalog = parseMsfCatalog(JSON.stringify({
            version: 1,
            format: 'cmsf', // extension field — silently ignored per §5.1 (unknown fields)
            tracks: [
                {
                    name: 'vide_1',
                    id: 1, // extension field — ignored
                    role: 'video',
                    packaging: 'cmaf',
                    renderGroup: 1,
                    isLive: false,
                ...
                },
                {
                    ...
                    isLive: false,
                    codec: 'mp4a.40.2',
                    sampleRate: 48000, // non-standard camelCase — ignored per unknown-field rule
                    channelCount: 2,   // non-standard field — ignored
                    initData: 'AAAAHGZ0eXBpc281',
                },
                {
                    name: 'vide_1_sap',
        ...
        // Video track
        const video = catalog.tracks[0]!;
        expect(video.name).toBe('vide_1');
        expect(video.role).toBe('video');
        expect(video.packaging).toBe('cmaf');
        expect(video.codec).toBe('avc1.64001F');

        // Audio track — non-standard sampleRate/channelCount fields are ignored
        const audio = catalog.tracks[1]!;
        expect(audio.name).toBe('soun_2');
        expect(audio.role).toBe('audio');
        expect(audio.packaging).toBe('cmaf');
        expect(audio.samplerate).toBeUndefined(); // non-standard 'sampleRate' not mapped
        expect(audio.channelConfig).toBeUndefined(); // non-standard 'channelCount' not mapped

        // SAP tracks
        const videoSap = catalog.tracks[2]!;
        expect(videoSap.name).toBe('vide_1_sap');
        expect(videoSap.packaging).toBe('eventtimeline');
        expect(videoSap.eventType).toBe('org.ietf.moq.cmsf.sap');
        expect(videoSap.depends).toEqual(['vide_1']);

        const audioSap = catalog.tracks[3]!;
        expect(audioSap.name).toBe('soun_2_sap');
        expect(audioSap.packaging).toBe('eventtimeline');
        expect(audioSap.eventType).toBe('org.ietf.moq.cmsf.sap');
        expect(audioSap.depends).toEqual(['soun_2']);

        // Verify SAP tracks are discoverable via filter on packaging + depends
        const sapTracks = catalog.tracks.filter(
            t => t.packaging === 'eventtimeline' &&
                 t.eventType === 'org.ietf.moq.cmsf.sap',
        );
        expect(sapTracks).toHaveLength(2);
        expect(sapTracks.map(t => t.depends).flat()).toEqual(['vide_1', 'soun_2']);
    });
});
