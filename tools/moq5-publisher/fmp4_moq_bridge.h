...
/* Prepare a live CMAF media sender. WebTransport CONNECT starts
 * immediately (overlaps remux probe). Sender attach still waits for the
 * first moov so the initial live catalog already has vide/soun + init
 * (not `{tracks:[]}`). opts may be NULL. Returns NULL on failure. */
